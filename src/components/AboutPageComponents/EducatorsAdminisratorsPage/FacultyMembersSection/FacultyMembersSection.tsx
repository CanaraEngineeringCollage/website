...
const [facultyData, setFacultyData] = useState([]);
  ...
  const filteredData =
    selectedCategory === "faculty"
      ...
      : selectedCategory === "placement"?facultyData.filter((item)=> item.department==="Placement Team")
    ...
    <div className="sticky top-20 h-fit">
          <div className="w-full sm:w-[80%] mx-auto md:mx-0">
            <div className="border-b-2 border-border pb-4 sm:pb-5">
              ...
              <h1
                className={`text-[20px] cursor-pointer ${selectedCategory === "placement" ? "font-bold text-[#2884CA]" : "text-textGray"}`}
                onClick={() => {
                  setSelectedCategory("placement");
        ...
        <div className="md:col-span-7 text-sm sm:text-base h-[90vh] scrollable overflow-y-auto  pr-2 md:text-lg">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg2:grid-cols-3 xl:grid-cols-3 gap-4 sm:gap-6 md:gap-3 justify-items-center">
            {selectedCategory != "faculty"&& selectedCategory != "placement"&&filteredData.map((item) => (
              <div
                key={item.id}
              ...
              >
                ...
                { selectedCategory === "placement"&&filteredData.map((item) => (
              <div
                key={item.id}
                onClick={() => {

...
const DepartmentSection = () => {
  return (
    <div className="max-w-7xl mx-auto lg:px-36 xl:max-w-[75%]  py-20 text-black">
      ...
      <h1 className="text-3xl md:text-[40px] lg2:text-5xl xl:text-6xl pt-10 font-bold">Aim:</h1>
      <p className="lg:text-xl md:text-lg text-[14px] leading-7 pt-10 text-textGray">
      ...
      <h1 className="text-3xl text-start md:text-[40px] lg2:text-5xl pt-10 xl:text-6xl font-bold">Objectives</h1>
      <ul className="lg:text-xl text-start list-disc ml-5 md:text-lg text-[14px] leading-7 py-10 text-textGray">
        {objectives.map((objective, index) => (
          <li>{objective}</li>
        ))}
      </ul>
      <h1 className="text-3xl text-start md:text-[40px] lg2:text-5xl pt-10 xl:text-6xl font-bold">Duties:</h1>
      <ul className="lg:text-xl text-start list-disc ml-5 md:text-lg text-[14px] leading-7 py-10 text-textGray">
      ...
      <ModelTable title="Sports Committee List" headers={["Sl No", "Name", "Designation", "Department"]} rows={members} />
      <ModelTable title="Sports facilities available in our college:" headers={["Sl No", "Sports", "No"]} rows={sportsFacilities} />
       <ModelTable title="Student participation in sports from 2017-18 to 2022-23:" headers={["Sl No", "Year", "Number of Events (State Level)","Number of Participants"]} rows={participation} />
    </div>
  );
};

...
export default function LibraryFaculty() {
  ...
  useEffect(() => {
    async function fetchFaculty() {
      setLoading(true);
      try {
        const url = `${process.env.NEXT_PUBLIC_API_URL}/faculty?department=${encodeURIComponent("Library")}&all=true`; // ✅ Add all=true to fetch all faculties
    ...
    <section className="py-16 px-6 md:px-12 max-w-7xl mx-auto mt-10  md:my-20 xl:max-w-[75%] bg-[#F5F5F7] md:rounded-3xl">
        ...
        <div className="max-w-md space-y-44">
          <div>
            <h2 className="text-3xl lg:text-4xl md:text-4xl font-bold text-gray-900 leading-tight">
              Get to Know Our <br />
              <span className="">Library Staff</span>
            </h2>
          ...
          <div className="flex items-center justify-between gap-4">
            <Link href="/about/educators-administrators?category=library">
              <button
                aria-label="Our Library Staff team"
                className="bg-blue-100 text-[#1D1D1F] rounded-full text-block px-6 py-2  text-sm font-medium hover:bg-blue-200 transition"
              >
      ...
      <div className="max-w-7xl mx-auto flex flex-col items-center justify-between gap-10 lg1:hidden">
        ...
        <div className="flex flex-col items-center gap-7">
          <Link href="/about/educators-administrators?category=library">
            {" "}
            <button className="bg-blue-100 rounded-full text-[#1D1D1F] text-block px-6 py-2  text-sm font-medium hover:bg-blue-200 transition">
              Our Library Staff Team
            </button>
          </Link>
